...
1. Read current state.
   - `python3 scripts/debug/overnight_ctl.py interactive-status --json`
...
2. Read the plan.
   ...
   - Re-import plan task cards when the plan changed:
     - `python3 scripts/debug/overnight_ctl.py queue-import-plan --project <project>`

3. Clean up stale claims.
   - `landed_clean`: release as `done`
   - missing or abandoned worktree: release as `ready` or `blocked`
   - use:
     - `python3 scripts/debug/overnight_ctl.py interactive-finish --project <project> --worktree <path> --state <state>`
