...
sync_remote_repo() {
  ...
  remote_shell "mkdir -p $(printf '%q' "$remote_repo_dir") && rm -rf $(printf '%q' "$remote_repo_dir")/.shadow-vm $(printf '%q' "$remote_repo_dir")/build"
  rsync -az --delete \
    --exclude='.git' \
    --exclude='.direnv' \
    --exclude='.devenv' \
...
pull_remote_artifacts() {
  ...
  rsync -az -e "$ssh_cmd" \
    ...
    "$(repo_root)/build/ci/" >/dev/null 2>&1 || true
  rsync -az -e "$ssh_cmd" \
    "${REMOTE_HOST}:$remote_repo_dir/build/ui-vm/" \
    "$(repo_root)/build/ui-vm/" >/dev/null 2>&1 || true
}
