...
MOCK_ANDROID_FONT_SOURCE_DIR="$TMP_DIR/android-fonts"
...
TS_EXTRA_RUST_APP_DIRECT_PRESENT_BUNDLE_DIR="$TMP_DIR/ts-extra-rust-app-direct-present-bundle"
...
mkdir -p \
  ...
  "$TS_EXTRA_RUST_APP_DIRECT_PRESENT_BUNDLE_DIR/lib" \
...
printf 'mock avb key\n' >"$AVB_KEY_PATH"
printf 'mock DroidSans\n' >"$MOCK_ANDROID_FONT_SOURCE_DIR/DroidSans.ttf"
printf 'mock DroidSans-Bold\n' >"$MOCK_ANDROID_FONT_SOURCE_DIR/DroidSans-Bold.ttf"
printf 'mock DroidSansMono\n' >"$MOCK_ANDROID_FONT_SOURCE_DIR/DroidSansMono.ttf"
printf 'mock NotoColorEmoji\n' >"$MOCK_ANDROID_FONT_SOURCE_DIR/NotoColorEmoji.ttf"
export PIXEL_ANDROID_FONT_SOURCE_DIR="$MOCK_ANDROID_FONT_SOURCE_DIR"
...
cp -R "$TS_APP_DIRECT_PRESENT_BUNDLE_DIR"/. "$TS_EXTRA_RUST_APP_DIRECT_PRESENT_BUNDLE_DIR"/
printf 'ELF_RUST_DEMO_AARCH64\n' >"$TS_EXTRA_RUST_APP_DIRECT_PRESENT_BUNDLE_DIR/shadow-rust-demo"
chmod 0755 "$TS_EXTRA_RUST_APP_DIRECT_PRESENT_BUNDLE_DIR/shadow-rust-demo"
...
assert_tar_xz_entry_present() {
  ...
  python3 - "$archive_path" "$tar_entry" <<'PY'
from pathlib import Path
import sys
import tarfile

archive_path, tar_entry = sys.argv[1:3]
with tarfile.open(Path(archive_path), mode="r:xz") as archive:
    names = {member.name.removeprefix("./") for member in archive.getmembers()}
if tar_entry not in names:
    raise SystemExit(f"missing tar.xz entry: {tar_entry}")
PY
...
assert_tar_xz_entry_contains() {
  local archive_path tar_entry expected_substring
  archive_path="$1"
  tar_entry="$2"
  expected_substring="$3"

  python3 - "$archive_path" "$tar_entry" "$expected_substring" <<'PY'
from pathlib import Path
import sys
import tarfile

archive_path, tar_entry, expected_substring = sys.argv[1:4]
with tarfile.open(Path(archive_path), mode="r:xz") as archive:
    members = {member.name.removeprefix("./"): member for member in archive.getmembers()}
    member = members.get(tar_entry)
    if member is None:
...
assert_cpio_tar_xz_entry_present "$TMP_DIR/orange-gpu-rust-bridge-ts-app-direct-present.img" orange-gpu.tar.xz system/fonts/DroidSans.ttf
...
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"startAppId": "counter"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"key": "SHADOW_RUNTIME_SESSION_CONFIG"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"value": "/orange-gpu/app-direct-present-startup.json"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"nostrDbPath": "/shadow-runtime/runtime-nostr.sqlite3"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"nostrServiceSocket": "/shadow-runtime/runtime-nostr.sock"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"exitOnFirstFrame": true'
...
assert_cpio_tar_xz_entry_present "$TMP_DIR/orange-gpu-rust-bridge-shell-session.img" orange-gpu.tar.xz system/fonts/DroidSans.ttf
...
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session.img" orange-gpu.tar.xz shell-session-startup.json '"shellStartAppId": "counter"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session.img" orange-gpu.tar.xz shell-session-startup.json '"key": "SHADOW_RUNTIME_SESSION_CONFIG"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session.img" orange-gpu.tar.xz shell-session-startup.json '"value": "/orange-gpu/shell-session-startup.json"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session.img" orange-gpu.tar.xz shell-session-startup.json '"key": "SHADOW_RUNTIME_APP_COUNTER_BUNDLE_PATH"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session.img" orange-gpu.tar.xz shell-session-startup.json '"nostrServiceSocket": "/shadow-runtime/runtime-nostr.sock"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session.img" orange-gpu.tar.xz shell-session-startup.json '"exitOnFirstFrame": true'
...
shell_session_shadow_logical_boot_output="$(
  env PATH="$MOCK_BIN:$PATH" SHADOW_BOOTIMG_SHELL=1 MOCK_BOOT_RAMDISK="$BOOT_BUILD_RAMDISK" \
    ...
    "$REPO_ROOT/scripts/pixel/pixel_boot_build_orange_gpu.sh" \
      ...
      --orange-gpu-metadata-stage-breadcrumb true \
      --firmware-bootstrap ramdisk-lib-firmware \
      --shell-session-extra-app-ids rust-demo \
...
shell_session_shadow_logical_archive="$TMP_DIR/orange-gpu-rust-bridge-shell-session-shadow-logical.img.orange-gpu.tar.xz"
assert_tar_xz_entry_present "$shell_session_shadow_logical_archive" app-direct-present/run-shadow-blitz-demo
assert_tar_xz_entry_present "$shell_session_shadow_logical_archive" app-direct-present/run-shadow-rust-demo
assert_tar_xz_entry_present "$shell_session_shadow_logical_archive" app-direct-present/shadow-blitz-demo
assert_tar_xz_entry_present "$shell_session_shadow_logical_archive" app-direct-present/shadow-rust-demo
assert_tar_xz_entry_present "$shell_session_shadow_logical_archive" app-direct-present/runtime-app-counter-bundle.js
assert_tar_xz_entry_present "$shell_session_shadow_logical_archive" system/fonts/DroidSans.ttf
assert_tar_xz_entry_contains "$shell_session_shadow_logical_archive" shell-session-startup.json '"key": "SHADOW_APP_DIRECT_PRESENT_BINARY_PATH"'
assert_tar_xz_entry_contains "$shell_session_shadow_logical_archive" shell-session-startup.json '"value": "/orange-gpu/app-direct-present/shadow-blitz-demo"'
assert_tar_xz_entry_contains "$shell_session_shadow_logical_archive" shell-session-startup.json '"key": "SHADOW_APP_CLIENT_RUST_DEMO"'
...
assert_cpio_tar_xz_entry_present "$TMP_DIR/orange-gpu-rust-bridge-shell-session-held.img" orange-gpu.tar.xz system/fonts/DroidSans.ttf
...
assert_cpio_tar_xz_entry_present "$TMP_DIR/orange-gpu-rust-bridge-shell-session-timeline.img" orange-gpu.tar.xz system/fonts/DroidSans.ttf
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-timeline.img" orange-gpu.tar.xz shell-session-startup.json '"shellStartAppId": "timeline"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-timeline.img" orange-gpu.tar.xz shell-session-startup.json '"key": "SHADOW_RUNTIME_SESSION_CONFIG"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-timeline.img" orange-gpu.tar.xz shell-session-startup.json '"key": "SHADOW_RUNTIME_APP_TIMELINE_BUNDLE_PATH"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-timeline.img" orange-gpu.tar.xz shell-session-startup.json '"nostrServiceSocket": "/shadow-runtime/runtime-nostr.sock"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-timeline.img" orange-gpu.tar.xz shell-session-startup.json '"exitOnFirstFrame": true'
...
assert_cpio_tar_xz_entry_present "$TMP_DIR/orange-gpu-rust-bridge-shell-session-touch-counter.img" orange-gpu.tar.xz system/fonts/DroidSans.ttf
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-touch-counter.img" orange-gpu.tar.xz shell-session-startup.json '"shellStartAppId": "counter"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-touch-counter.img" orange-gpu.tar.xz shell-session-startup.json '"key": "SHADOW_RUNTIME_SESSION_CONFIG"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-touch-counter.img" orange-gpu.tar.xz shell-session-startup.json '"key": "SHADOW_BLITZ_TOUCH_ANYWHERE_TARGET"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-touch-counter.img" orange-gpu.tar.xz shell-session-startup.json '"nostrServiceSocket": "/shadow-runtime/runtime-nostr.sock"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-touch-counter.img" orange-gpu.tar.xz shell-session-startup.json '"exitOnFirstFrame": false'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-shell-session-touch-counter.img" orange-gpu.tar.xz shell-session-startup.json '"exitAfterPresent": true'
...
assert_cpio_tar_xz_entry_present "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter.img" orange-gpu.tar.xz system/fonts/DroidSans.ttf
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter.img" orange-gpu.tar.xz app-direct-present-startup.json '"startAppId": "counter"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter.img" orange-gpu.tar.xz app-direct-present-startup.json '"key": "SHADOW_RUNTIME_SESSION_CONFIG"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter.img" orange-gpu.tar.xz app-direct-present-startup.json '"key": "SHADOW_BLITZ_TOUCH_ANYWHERE_TARGET"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter.img" orange-gpu.tar.xz app-direct-present-startup.json '"nostrServiceSocket": "/shadow-runtime/runtime-nostr.sock"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter.img" orange-gpu.tar.xz app-direct-present-startup.json '"exitOnFirstFrame": false'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter.img" orange-gpu.tar.xz app-direct-present-startup.json '"exitAfterPresent": true'
...
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter-manual.img" orange-gpu.tar.xz app-direct-present-startup.json '"startAppId": "counter"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter-manual.img" orange-gpu.tar.xz app-direct-present-startup.json '"key": "SHADOW_RUNTIME_SESSION_CONFIG"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter-manual.img" orange-gpu.tar.xz app-direct-present-startup.json '"key": "SHADOW_BLITZ_TOUCH_ANYWHERE_TARGET"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter-manual.img" orange-gpu.tar.xz app-direct-present-startup.json '"nostrServiceSocket": "/shadow-runtime/runtime-nostr.sock"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter-manual.img" orange-gpu.tar.xz app-direct-present-startup.json '"exitOnFirstFrame": false'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-runtime-touch-counter-manual.img" orange-gpu.tar.xz app-direct-present-startup.json '"exitAfterPresent": true'
...
assert_cpio_tar_xz_entry_present "$TMP_DIR/orange-gpu-rust-bridge-ts-timeline-app-direct-present.img" orange-gpu.tar.xz system/fonts/DroidSans.ttf
...
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-timeline-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"startAppId": "timeline"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-timeline-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"key": "SHADOW_RUNTIME_SESSION_CONFIG"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-timeline-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"value": "/orange-gpu/app-direct-present-startup.json"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-timeline-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"nostrDbPath": "/shadow-runtime/runtime-nostr.sqlite3"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-timeline-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"nostrServiceSocket": "/shadow-runtime/runtime-nostr.sock"'
assert_cpio_tar_xz_entry_contains "$TMP_DIR/orange-gpu-rust-bridge-ts-timeline-app-direct-present.img" orange-gpu.tar.xz app-direct-present-startup.json '"exitOnFirstFrame": true'
...
assert_command_fails_contains "orange gpu mode must be gpu-render, orange-gpu-loop, bundle-smoke, vulkan-instance-smoke, raw-vulkan-instance-smoke, firmware-probe-only, timeout-control-smoke, camera-hal-link-probe, wifi-linux-surface-probe, c-kgsl-open-readonly-smoke, c-kgsl-open-readonly-firmware-helper-smoke, c-kgsl-open-readonly-pid1-smoke, raw-kgsl-open-readonly-smoke, raw-kgsl-getproperties-smoke, raw-vulkan-physical-device-count-query-exit-smoke, raw-vulkan-physical-device-count-query-no-destroy-smoke, raw-vulkan-physical-device-count-query-smoke, raw-vulkan-physical-device-count-smoke, vulkan-enumerate-adapters-count-smoke, vulkan-enumerate-adapters-smoke, vulkan-adapter-smoke, vulkan-device-request-smoke, vulkan-device-smoke, vulkan-offscreen, compositor-scene, shell-session, shell-session-held, shell-session-runtime-touch-counter, app-direct-present, app-direct-present-touch-counter, app-direct-present-runtime-touch-counter, or payload-partition-probe" \
  env PATH="$MOCK_BIN:$PATH" SHADOW_BOOTIMG_SHELL=1 MOCK_BOOT_RAMDISK="$BOOT_BUILD_RAMDISK" \
    PIXEL_ROOT_STOCK_BOOT_IMG="$BOOT_BUILD_INPUT" \
    "$REPO_ROOT/scripts/pixel/pixel_boot_build_orange_gpu.sh" \
      --input "$BOOT_BUILD_INPUT" \
