...
assert_contains "$camera_hal_probe_dry_run_output" "camera_id=0"
assert_contains "$camera_hal_probe_dry_run_output" "camera_hal_call_open=false"
assert_contains "$camera_hal_probe_dry_run_output" "camera_linker_capsule_mode=auto"
assert_contains "$camera_hal_probe_dry_run_output" "camera_linker_capsule_dir=$CAMERA_HAL_PROBE_OUTPUT/camera-linker-capsule"
assert_contains "$camera_hal_probe_dry_run_output" "camera_linker_capsule_include_components=false"
assert_contains "$camera_hal_probe_dry_run_output" "capsule_command=$REPO_ROOT/scripts/pixel/pixel_camera_hal_collect_capsule.sh --output \"$CAMERA_HAL_PROBE_OUTPUT/camera-linker-capsule\" --include-camera-components false"
assert_contains "$camera_hal_probe_dry_run_output" "build_command=$REPO_ROOT/scripts/pixel/pixel_boot_build_orange_gpu.sh --output \"$CAMERA_HAL_PROBE_OUTPUT/camera-boot-hal.img\" --orange-gpu-mode camera-hal-link-probe --orange-gpu-metadata-stage-breadcrumb true --run-token \"camera-hal-"
assert_contains "$camera_hal_probe_dry_run_output" "--camera-linker-capsule \"$CAMERA_HAL_PROBE_OUTPUT/camera-linker-capsule\""
