...
def generate_combined_rust(manifest: dict[str, Any]) -> str:
    apps = manifest["apps"]
    icon_colors = sorted({app["ui"]["iconColor"] for app in apps})
    super_imports = ["AppId", "AppLaunchEnv", "AppModel", "DemoApp", "ManifestAppLaunch"]
...
def main() -> int:
    ...
    if args.launch_rust_out is not None or uses_default_rust_out(args.rust_out):
        launch_rust_out = args.launch_rust_out or default_launch_rust_out_path()
    ...
    return check_or_write(
        args.rust_out,
        generate_combined_rust(manifest),
        check=args.check,
    )
