...
def default_launch_rust_out_path() -> Path:
    return (
        repo_root()
        / "ui"
        / "crates"
...
def generate_apps_rust(manifest: dict[str, Any]) -> str:
    apps = manifest["apps"]
    ...
    lines: list[str] = [
        ...
        "use super::{AppId, AppModel, DemoApp};",
    ...
    for app in apps:
        ...
        lines.extend(
            [
                ...
                f"    manifest_launch: super::{rust_const_name(app_id, 'MANIFEST_LAUNCH')},",
...
def generate_launch_rust(manifest: dict[str, Any]) -> str:
    apps = manifest["apps"]
    super_imports = ["ManifestAppLaunch"]
    if any(app["model"] == "typescript" for app in apps):
        super_imports.append("AppRuntimeMetadata")
    if any(app.get("launchEnv") for app in apps):
        super_imports.append("AppLaunchEnv")
    lines: list[str] = [
        "// @generated by scripts/runtime/generate_app_metadata.py; do not edit by hand.",
    ...
    for app in apps:
        ...
        model = app["model"]
        runtime = app.get("runtime")
        launch_const = rust_const_name(app_id, "MANIFEST_LAUNCH")
        if model == "typescript":
            assert isinstance(runtime, dict)
            runtime_const = rust_const_name(app_id, "MANIFEST_RUNTIME")
            lines.extend(
                [
                    f"pub const {runtime_const}: AppRuntimeMetadata = AppRuntimeMetadata {{",
                    f"    bundle_env: {rust_str(runtime['bundleEnv'])},",
                    f"    input_path: {rust_str(runtime['inputPath'])},",
                    f"    cache_dir: {rust_str(rust_runtime_cache_dir(runtime))},",
                    "};",
                    "",
                ]
            )
        ...
        if model == "typescript":
            lines.extend(
                [
                    f"pub const {launch_const}: ManifestAppLaunch = ManifestAppLaunch::TypeScript {{",
                    f"    runtime: {runtime_const},",
            ...
            lines.extend(
                [
                    f"pub const {launch_const}: ManifestAppLaunch = ManifestAppLaunch::Rust {{",
                    f"    launch_env: {launch_env_ref},",
                    "};",
                    "",
                ]
...
def check_or_write_many(
    ...
    *,
    ...
    for path, content in outputs:
        status |= check_or_write(path, content, check=check)
    return status


def uses_default_rust_out(path: Path) -> bool:
    return path.resolve() == default_rust_out_path().resolve()


def resolve_launch_rust_out(rust_out: Path, launch_rust_out: Path | None) -> Path:
    if launch_rust_out is None:
        if uses_default_rust_out(rust_out):
            return default_launch_rust_out_path()
        raise SystemExit(
            "generate_app_metadata: --launch-rust-out is required when "
            "--rust-out is set to a non-default path"
        )
    if launch_rust_out.resolve() == rust_out.resolve():
        raise SystemExit(
            "generate_app_metadata: --rust-out and --launch-rust-out must be "
            "different paths"
        )
    return launch_rust_out
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--launch-rust-out", type=Path)
...
def main() -> int:
    ...
    launch_rust_out = resolve_launch_rust_out(args.rust_out, args.launch_rust_out)
    return check_or_write_many(
        [
            (args.rust_out, generate_apps_rust(manifest)),
            (launch_rust_out, generate_launch_rust(manifest)),
        ],
        check=args.check,
    )
