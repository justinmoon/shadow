...
Usage: scripts/pixel/pixel_boot_build_orange_gpu.sh [--input PATH] [--init PATH]
                                                    ...
                                                    [--prelude-hold-secs N]
                                                    [--orange-gpu-mode gpu-render|bundle-smoke|vulkan-offscreen]
...
shadow-gpu-smoke bundle under /orange-gpu for one of three rungs: the real GPU
render/present path, a strict Vulkan offscreen render path, or the no-Vulkan
...
assert_orange_gpu_mode_word() {
  ...
  case "$value" in
    gpu-render|bundle-smoke|vulkan-offscreen)
      ;;
    *)
      echo "pixel_boot_build_orange_gpu: orange gpu mode must be gpu-render, bundle-smoke, or vulkan-offscreen: $value" >&2
      exit 1
      ;;
  esac
}
