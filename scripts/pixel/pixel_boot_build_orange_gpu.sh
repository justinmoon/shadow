...
Usage: scripts/pixel/pixel_boot_build_orange_gpu.sh [--input PATH] [--init PATH]
                                                    ...
                                                    [--prelude-hold-secs N]
                                                    [--orange-gpu-mode gpu-render|bundle-smoke|vulkan-instance-smoke|raw-vulkan-instance-smoke|vulkan-enumerate-adapters-count-smoke|vulkan-enumerate-adapters-smoke|vulkan-adapter-smoke|vulkan-device-request-smoke|vulkan-device-smoke|vulkan-offscreen]
...
shadow-gpu-smoke bundle under /orange-gpu for one of ten rungs: the real GPU
render/present path, a strict Vulkan instance smoke, a strict raw Vulkan
instance smoke, a strict Vulkan raw adapter-enumeration-count smoke, a strict
Vulkan adapter-enumeration smoke, a strict Vulkan adapter smoke, a strict
Vulkan device-request smoke, a strict Vulkan device/buffer smoke, a strict
Vulkan offscreen render path, or the no-Vulkan bundle-exec smoke path.
EOF
}
...
assert_orange_gpu_mode_word() {
  ...
  case "$value" in
    gpu-render|bundle-smoke|vulkan-instance-smoke|raw-vulkan-instance-smoke|vulkan-enumerate-adapters-count-smoke|vulkan-enumerate-adapters-smoke|vulkan-adapter-smoke|vulkan-device-request-smoke|vulkan-device-smoke|vulkan-offscreen)
      ;;
    *)
      echo "pixel_boot_build_orange_gpu: orange gpu mode must be gpu-render, bundle-smoke, vulkan-instance-smoke, raw-vulkan-instance-smoke, vulkan-enumerate-adapters-count-smoke, vulkan-enumerate-adapters-smoke, vulkan-adapter-smoke, vulkan-device-request-smoke, vulkan-device-smoke, or vulkan-offscreen: $value" >&2
      exit 1
      ;;
  esac
}
