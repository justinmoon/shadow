...
compositor_binary_name="$(pixel_runtime_compositor_binary_name)"
compositor_launcher_name="$(pixel_runtime_compositor_launcher_name)"
compositor_out_link="$(pixel_dir)/${compositor_binary_name}-aarch64-linux-gnu-result"
compositor_launcher_artifact="$host_bundle_dir/$compositor_launcher_name"
...
audio_package_ref="$repo#packages.${linux_system}.shadow-audio-bridge-aarch64-linux-gnu"
audio_out_link="$(pixel_dir)/shadow-audio-bridge-aarch64-linux-gnu-result"
audio_binary_name="shadow-audio-bridge"
...
EOF
  if [[ "$audio_enabled" == "1" ]]; then
    cat >>"$host_launcher_artifact" <<EOF
export SHADOW_RUNTIME_AUDIO_BRIDGE_BINARY="\$DIR/$audio_binary_name"
export SHADOW_RUNTIME_AUDIO_BRIDGE_STAGE_LOADER_PATH="\$DIR/lib/$PIXEL_RUNTIME_STAGE_LOADER_NAME"
export SHADOW_RUNTIME_AUDIO_BRIDGE_STAGE_LIBRARY_PATH="\$DIR/lib"
export SHADOW_RUNTIME_AUDIO_SPIKE_BINARY="\$SHADOW_RUNTIME_AUDIO_BRIDGE_BINARY"
export SHADOW_RUNTIME_AUDIO_SPIKE_STAGE_LOADER_PATH="\$SHADOW_RUNTIME_AUDIO_BRIDGE_STAGE_LOADER_PATH"
export SHADOW_RUNTIME_AUDIO_SPIKE_STAGE_LIBRARY_PATH="\$SHADOW_RUNTIME_AUDIO_BRIDGE_STAGE_LIBRARY_PATH"
export ALSA_CONFIG_PATH="\$DIR/share/alsa/alsa.conf"
export ALSA_CONFIG_DIR="\$DIR/share/alsa"
export ALSA_CONFIG_UCM="\$DIR/share/alsa/ucm"
export ALSA_CONFIG_UCM2="\$DIR/share/alsa/ucm2"
