...
- `runtime-audio-host` now owns the first durable contract: `createPlayer`, `play`, `pause`, `stop`, `release`, and `getStatus`, with a memory backend on host and a `linux_bridge` backend on the rooted Pixel lane.
- The current rooted Pixel proof is now app-level and audible: the sound demo auto-clicked `play`, `Shadow.os.audio` spawned `shadow-audio-bridge`, and the device speaker emitted the tone during the rooted runtime session.
...
- `just runtime-app-host-smokes` now covers two sound host contracts: the normal `memory` backend UI flow and a fake `linux_bridge` helper that writes junk to stdout, so stdio pollution in the audio helper path fails locally instead of waiting for a Pixel run.
- The packaged helper has graduated to the shared `shadow-audio-bridge` binary/package name. The old `shadow-linux-audio-spike` package attr, `linux_spike` backend value, and `SHADOW_RUNTIME_AUDIO_SPIKE_*` env names remain compatibility aliases during the transition.
- Boot shell/app payloads now have an opt-in `PIXEL_ORANGE_GPU_ENABLE_LINUX_AUDIO=true` path that stages the same `shadow-audio-bridge` package, ALSA config/plugins, and `services.audioBackend = "linux_bridge"` into the orange GPU bundle without changing default compositor/session bring-up.
