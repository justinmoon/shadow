...
#[cfg(android_ndk)]
type MarkVintfStabilityFn = unsafe extern "C" fn(*mut sys::AIBinder);

#[cfg(android_ndk)]
static MARK_VINTF_STABILITY: OnceLock<Option<MarkVintfStabilityFn>> = OnceLock::new();
...
impl<T: Remotable> Binder<T> {
    ...
    /// Mark this binder object with the given stability guarantee.
    ///
    /// For Android NDK Cargo builds, local stability is still applied lazily by
    /// libbinder when the binder is first flattened into a parcel. VINTF
    /// stability must be set explicitly before that first send.
    #[cfg(android_ndk)]
    fn mark_stability(&mut self, stability: Stability) {
        match stability {
            Stability::Local => {}
            Stability::Vintf => {
                let Some(mark_vintf_stability) =
                    MARK_VINTF_STABILITY.get_or_init(load_mark_vintf_stability).as_ref()
                else {
                    return;
                };
                unsafe {
                    mark_vintf_stability(self.as_native_mut());
                }
            }
        }
    }
...
#[cfg(android_ndk)]
fn load_mark_vintf_stability() -> Option<MarkVintfStabilityFn> {
    let library_name = b"libbinder_ndk.so\0";
    let handle = unsafe {
        libc::dlopen(
    ...
    let symbol_name = b"AIBinder_markVintfStability\0";
    unsafe {
        let _ = libc::dlerror();
        let symbol = libc::dlsym(handle, symbol_name.as_ptr().cast());
        if symbol.is_null() {
            None
        } else {
