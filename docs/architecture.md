...
19. `just runtime-app-cashu-wallet-smoke` proves the second real host domain. It boots a local `cdk-mintd` fakewallet mint, trusts that mint through `Shadow.os.cashu`, mints a Lightning invoice into balance, sends and receives a Cashu token, then pays a generated Lightning invoice and remints it back into wallet state.
20. `just pixel-shell-drm` is now the primary rooted Pixel shell rung: stage the counter, timeline, podcast, and Cashu runtime bundles plus the GNU-wrapped helper, boot the guest compositor into the home scene, and present the real shell/home path on the phone panel. `just run app=<app> target=pixel` now uses that same shell rung and asks the shell to open the selected app.
21. `shadowctl --target <serial> state|open|home|switcher` is the rooted Pixel shell control seam: it talks to the guest compositor's `shadow-control.sock` over rooted `adb shell` plus `nc -U`, so operators can query state, go home, or launch an app without falling back to touch-only QA.
22. `just pixel-shell-timeline-smoke` is the first rooted lifecycle proof on that shell lane: start `timeline`, go home, and reopen it through the live shell control path.
23. `just pixel-runtime-app-drm` remains as a narrower fallback/probe rung for direct runtime-app work on the rooted phone: fit the shell app viewport contract into the real panel, pass the same fitted size to both the guest compositor and the runtime client, and prove the runtime-mode Blitz demo reaches the real panel.
24. `just pixel-runtime-app-click-drm` proves the rooted panel path survives one runtime click dispatch and rerender before Android display services are restored.
...
8. `just runtime-app-cashu-wallet-smoke` proves the second real host domain. It boots a local `cdk-mintd` fakewallet mint, trusts that mint through `Shadow.os.cashu`, mints a Lightning invoice into balance, sends and receives a Cashu token, then pays a generated Lightning invoice and remints it back into wallet state.
9. The older split `document` / `click` / `input` / `focus` / `toggle` / `selection` host recipes are no longer part of the live operator surface; the public proofs are the app-level host smokes above.
10. `shadow-compositor-guest` now also advertises a small linux-dmabuf global and logs the type of each committed client buffer. The first guest-compositor GPU smoke proved the compositor can classify that path, but on the current headless Linux host the static GPU Blitz demo still lands as `type=shm`, not `type=dma`.
11. The rooted Pixel now has a working static `wgpu` smoke too: `scripts/pixel_blitz_demo_static_drm_gpu_softbuffer.sh` pushes a GNU-wrapped Blitz demo, launches it under `shadow-compositor-guest`, and proves the guest compositor can capture and present `384x720` frames on the phone panel.
...
15. `just pixel-drm-probe` still explains why the original DRM-oriented Turnip path was brittle: on the rooted Pixel 4a, both `/dev/dri/card0` and `/dev/dri/renderD128` report `name=msm_drm` with `DRM_CAP_SYNCOBJ=0`, and `DRM_CAP_SYNCOBJ_TIMELINE` returns `EINVAL`. That matches Turnip's DRM syncobj requirement and explains why the successful path is the KGSL-preferred Vulkan lane, not the default DRM render-node lane.
16. The first GPU-finish observability rung is now landed too: the Blitz client emits compact `gpu-summary-start` / `gpu-summary-client` markers, rooted runs write `gpu-summary.json`, and both static and runtime GPU runs now record enough evidence to classify the backend, adapter, software-vs-hardware state, `/dev/dri` vs `/dev/kgsl` activity, first-visible-frame latency, and click-to-updated-frame latency without hand-parsing raw logs.
17. The compositor still observes client buffers as `type=shm` on the successful GPU path, so the next renderer-quality seam is transport/import quality rather than “can the rooted Pixel do hardware-backed rendering at all?”
...
20. The rooted Pixel now also has a real shell/home rung. `scripts/pixel_shell_drm.sh` stages the counter + timeline + podcast + Cashu bundles plus the runtime host helper, sets `SHADOW_GUEST_START_APP_ID=shell`, and boots the guest compositor into the home scene instead of the older direct-runtime timeline path. When `PIXEL_SHELL_START_APP_ID` is present, the guest compositor stays in shell mode and then launches the requested runtime app through that same shell path.
21. The rooted Pixel shell lane now rides the same unified control CLI too. `scripts/shadowctl -t <serial> state|open|home|switcher` resolves the rooted runtime dir, connects to `shadow-control.sock` with Toybox `nc -U`, and can return raw or JSON control state for live launch/home/reopen proofs.
22. Host scroll already rides the native Blitz path: `WindowEvent::MouseWheel` becomes `UiEvent::Wheel`, overflow containers scroll without a runtime JSON event, and the runtime wrapper now cancels synthetic click dispatch after a drag / pan gesture crosses a small movement threshold.
23. Runtime audio now has its first vertical seam too. `runtime/app-runtime/shadow_runtime_os.js` exports `Shadow.os.audio`, `rust/runtime-audio-host` injects that API into the embedded runtime host, host smokes use a small in-memory backend, and the rooted Pixel sound app flips to a sound-only no-`chroot` launcher so a spawned Linux helper can still reach `/dev/snd`.
24. The first staged file-backed proof is now landed on top of that seam. `scripts/prepare_sound_demo_assets.sh` validates the checked-in MP3 fixture under `runtime/app-sound-smoke/assets/`, the runtime sound app receives that as a bundle-relative `file` source through `SHADOW_RUNTIME_APP_CONFIG_JSON`, `runtime-audio-host` resolves it via `SHADOW_RUNTIME_BUNDLE_DIR`, and the Linux helper decodes the file in-process before writing PCM to ALSA.
