...
- Replace ad hoc launch-time env assembly with a small typed config loaded once at startup. The VM lane now does this through a generated `session-config.json`, and the supported rooted-Pixel guest path now stages a typed `guest-startup.json` artifact with env reserved for process-boundary values and compatibility/debug overlays.
