...
21. `just pixel-shellctl ...` is the rooted Pixel shell control seam: it talks to the guest compositor's `shadow-control.sock` over `adb shell` plus `nc -U`, so operators can query state, go home, or launch an app without falling back to touch-only QA.
...
21. The rooted Pixel shell lane now has a matching control seam too. `scripts/pixel_shellctl.sh` resolves the rooted runtime dir, connects to `shadow-control.sock` with Toybox `nc -U`, and can return raw or JSON control state for live launch/home/reopen proofs.
