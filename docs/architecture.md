...
26. That sound lane is no longer tone-only. The sound demo now stages one checked-in MP3 fixture into the runtime bundle, passes a bundle-relative `source.kind="file"` path through the same OS API seam, and rooted Pixel auto-click runs prove the runtime host can spawn the file-backed Linux helper and reach audible playback on the real panel path.
...
23. The first staged file-backed proof is now landed on top of that seam. `scripts/prepare_sound_demo_assets.sh` copies a deterministic repo MP3 fixture into `audio/demo-tone.mp3`, the runtime sound app receives that as a bundle-relative `file` source through `SHADOW_RUNTIME_APP_CONFIG_JSON`, `runtime-audio-host` resolves it via `SHADOW_RUNTIME_BUNDLE_DIR`, and the Linux helper decodes the file in-process before writing PCM to ALSA.
