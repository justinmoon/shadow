...
- `sound` and `podcast` additionally write `audio-proof.json` and `audio-spike-summary.json` in their `build/pixel/drm-guest/<run>/` directories. The lane is green only when the Linux audio bridge proves file-backed ALSA playback on a non-proxy PCM route.
