...
- `sound` additionally writes `audio-proof.json` and `audio-spike-summary.json` in its `build/pixel/drm-guest/<run>/` directory. The lane is green only when the Linux audio bridge proves file-backed ALSA playback on a non-proxy PCM route.
