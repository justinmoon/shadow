...
impl MasonryState<'_> {
    pub fn new(
        ...
        MasonryState {
            ...
            clipboard_cx: clipboard_context(),
...
fn clipboard_context() -> Box<dyn ClipboardProvider> {
    match ClipboardContext::new() {
        Ok(context) => Box::new(context),
        Err(error) => {
            warn!(
                error = %error,
                "masonry clipboard unavailable; using no-op clipboard provider"
            );
            Box::new(
                NopClipboardContext::new()
                    .expect("no-op clipboard context should always initialize"),
            )
        }
    }
}
