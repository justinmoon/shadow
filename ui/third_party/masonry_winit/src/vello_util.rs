...
impl RenderContext {
    pub(crate) fn new() -> Self {
        ...
        if backends.is_empty() {
            eprintln!(
                "shadow-masonry-wgpu: empty WGPU_BACKEND selection; falling back to default backends"
            );
        ...
        eprintln!(
            "shadow-masonry-wgpu: create-instance backends={:?} enabled={:?} WGPU_BACKEND={:?} VK_ICD_FILENAMES={:?} LD_LIBRARY_PATH={:?}",
            backends,
    ...
    async fn new_device(&mut self, compatible_surface: Option<&Surface<'_>>) -> Option<usize> {
        ...
        eprintln!(
            "shadow-masonry-wgpu: selected-adapter backend={:?} device_type={:?} name={:?} driver={:?} driver_info={:?}",
