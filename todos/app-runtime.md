...
- [x] Rooted-Pixel GPU viability decision.
  The rooted Pixel now has a real hardware-backed GPU path under the current rooted GNU userspace, but it is narrower than the original “just flip to GPU” plan. Current state:
  - `WGPU_BACKEND=gl` on the rooted Pixel still falls back to software. The static `gl` probe opens `/dev/dri/renderD128` and `/dev/dri/card0`, never opens `/dev/kgsl-3d0`, logs `libEGL ... failed to create dri2 screen`, lands on compositor-observed `type=shm`, and takes about `11.6s` to first visible frame. That is not the path we want.
  - The successful path is vendor Turnip + Vulkan + KGSL + `gpu_softbuffer`. The fixed static probe now reports `adapter_name="Turnip Adreno (TM) 618"`, `driver="turnip Mesa driver"`, `hardware_backed=true`, and `openlog_kgsl_seen=true` on the real Pixel.
  - The runtime app lane now works on that same GPU path too. `scripts/pixel_runtime_app_drm.sh` defaults to `gpu_softbuffer` when the cached vendor Turnip tarball is present, scrubs the GPU loader env before spawning the GNU runtime host, and the rooted auto-click lane now reports `hardware_backed=true`, `first_visible_frame_ms=943`, and `click_to_updated_frame_ms=38`.
  - The compositor still observes client buffers as `type=shm`, so the next GPU-quality seam is transport/import quality rather than “can we get a real adapter at all?”
  - `just pixel-drm-probe` still matters: the phone’s DRM render node reports `syncobj=0`, which explains why the original DRM-oriented Turnip path was so brittle. The successful path is the KGSL-preferred Vulkan lane instead.
  - Phase 1 observability is now in place too: the client emits compact `gpu-summary-*` markers, each rooted runtime run writes `gpu-summary.json`, and the probe path records openlog evidence for `/dev/dri` vs `/dev/kgsl`, compositor buffer type, first-visible-frame latency, and click-to-updated-frame latency.
