...
- [x] Rooted-Pixel GPU viability decision.
  ...
  - `WGPU_BACKEND=gl` works on the rooted Pixel through the softbuffer path, but the current static probe now shows the real shape of that lane: it opens `/dev/dri/renderD128` and `/dev/dri/card0`, never opens `/dev/kgsl-3d0`, logs `libEGL ... failed to create dri2 screen`, lands on compositor-observed `type=shm`, and takes about `11.6s` to first visible frame. That is a software fallback lane, not usable hardware acceleration.
  - `WGPU_BACKEND=vulkan` no longer fails only as a vague Turnip guess. The current static `vulkan_drm` probe now reaches the client and fails with `NoCompatibleDevice`, while the `vulkan_kgsl_first` probe now proves `/dev/kgsl-3d0` is reachable from the GNU client but still ends in the same `NoCompatibleDevice` failure. So the blocker is now more precise: the rooted GNU userspace can see KGSL, but `wgpu` / Vello still cannot create a usable hardware adapter there.
  ...
  - Phase 1 observability is now in place too: the client emits compact `gpu-summary-*` markers, each rooted runtime run writes `gpu-summary.json`, and the static GPU probe now also records openlog evidence for `/dev/dri` vs `/dev/kgsl`, EGL fallback warnings, compositor buffer type, and first-visible-frame latency so the next seam is driven by facts instead of raw log archaeology.
