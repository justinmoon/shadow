...
- Persistent audio daemon, shared socket protocol, and multi-app mixing.
- Real on-device network streaming until the Wi-Fi lane lands.
...
- Rooted Pixel playback should stay ALSA-first; do not revive Android bridge or `app_process` fallback plans for this proof.
...
- A persistent audio daemon with a socket is the clean long-term improvement for real pause/resume and multi-track mixing, but it is not a blocker for the current hardware proof.
