...
- The launch artifact/env contract still carries `runtimeHost*` and `SHADOW_RUNTIME_HOST_BINARY_PATH` compatibility names even though the actual package and binary are now `shadow-system`. Rename that schema only as a dedicated follow-up seam after all consumers are ready.
