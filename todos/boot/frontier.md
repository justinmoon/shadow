...
- C has reached its intended stopping point:
  ...
  - Raw-argv exec GPU proofs on 2026-04-22:
    ...
    - `orange-gpu-loop` is now signed off on the promoted Rust seam:
      - image metadata: `/Users/justin/code/shadow/worktrees/rust-boot/build/pixel/boot/shadow-boot-orange-gpu-rust-bridge-default-orange-gpu-loop-parentprobe-fw-helper-breadcrumb-v2.img.hello-init.json`
      - proof bundles: `/Users/justin/code/shadow/worktrees/rust-boot/build/pixel/boot/oneshot/orange-gpu-loop-v2-primary-11151JEC200472/recover-traces/status.json` and `/Users/justin/code/shadow/worktrees/rust-boot/build/pixel/boot/oneshot/orange-gpu-loop-v2-confirm-06241JEC200520/recover-traces/status.json`
      - both recovered bundles now report `proof_ok=true`, `probe_report_proves_child_success=true`, `probe_summary_proves_orange_gpu_loop=true`, `metadata_stage_value=parent-probe-result=exit-0`, `metadata_probe_stage_value=orange-gpu-payload:firmware-probe-ok`, `metadata_probe_report_child_exit_status=0`, and `metadata_probe_summary_kms_present.present_count=11`
      - the recovered loop summaries at `/Users/justin/code/shadow/worktrees/rust-boot/build/pixel/boot/oneshot/orange-gpu-loop-v2-primary-11151JEC200472/recover-traces/channels/metadata-probe-summary.json` and `/Users/justin/code/shadow/worktrees/rust-boot/build/pixel/boot/oneshot/orange-gpu-loop-v2-confirm-06241JEC200520/recover-traces/channels/metadata-probe-summary.json` both preserve `mode=orange-gpu-loop`, `frames_rendered=11`, `scanout_updates=11`, `distinct_frame_count=2`, and `kms_present.hold_secs=3`
      - this rung now proves repeated GPU render plus repeated KMS re-present on the same signed-off `probe-summary.json` / `recover-traces/status.json` contract rather than just one lucky frame
