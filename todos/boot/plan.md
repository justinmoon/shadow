...
- [ ] Prove boot-owned raw Vulkan physical-device enumeration count and return (`boot-raw-vulkan-physical-device-count-smoke`):
  - reuse the staged `shadow-gpu-smoke` bundle and the same strict env/bootstrap path
  - load Vulkan directly, create an instance, and stop immediately after `vkEnumeratePhysicalDevices` returns and the count is recorded
...
- [ ] Package the boot-owned raw Vulkan physical-device-count payload (`boot-raw-vulkan-physical-device-count-smoke`):
  ...
  - stop immediately after the raw physical-device count is recorded, before physical-device property extraction, all wgpu adapter enumeration/selection, `request_device`, buffer-renderer allocation, Vello render-to-texture, and KMS present
  - reuse the visible `orange-init` prelude/checkpoint/postlude contract to encode success on hardware
...
- Tightened inference after the bundle/offscreen/device-request/device/adapter/instance/raw-Vulkan proofs:
  ...
  - the next discriminator is `boot-raw-vulkan-physical-device-count-smoke`, reusing the same bundle and visible contract but stopping before any wgpu adapter enumeration
