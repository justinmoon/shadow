...
- [x] `payload-metadata-shell-bundle-probe`
  ...
  - notes:
    ...
    - Held-session proof: `ss-held-rust-logical-r1-20260424061149` kept shell-launched Rust `rust-demo` alive in boot-owned userspace until the 120s watchdog recovery path, then recovered `proof_ok=true`, `probe_summary_proves_shell_session_held=true`, `probe_report_proves_child_timeout=true`, and `metadata_probe_stage_value=orange-gpu-payload:shell-session-held-watchdog-proved`.
    - Operator-window proof: `ss-held-rust-hold-logical-r1-20260424062746` added `--orange-gpu-timeout-action hold`, recorded held-session proof at the 60s watchdog, left the compositor/Rust app running for the configured 60s observation window, returned through fastboot after 138s, and recovered `proof_ok=true`.
    - TypeScript interaction proof: `ss-touch-counter-logical-r1-20260424063926` staged the shell/runtime bundle in `shadow_payload_a`, launched the TypeScript `counter` app from the GPU shell, dispatched synthetic compositor touch, proved counter increment plus post-touch frame capture, and recovered `proof_ok=true`.
    - Touch input bootstrap proof: `ss-touch-input-logical-r1-20260424064945` repeated the logical TypeScript counter interaction with `sunfish-touch-event2`, worker-2's `heatmap.ko`/`ftm5.ko`, and combined GPU/touch firmware; recovered logs show `touch-ready device=/dev/input/event2 name=fts`, while the deterministic synthetic tap proof still recovered `proof_ok=true`.
  - owned paths:
    - `scripts/pixel/`
    - `scripts/lib/pixel_runtime_linux_bundle_common.sh`
