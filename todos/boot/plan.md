...
- Tightened inference after the bundle/offscreen/device-request/device/adapter/instance/raw-Vulkan proofs:
  ...
  - the next discriminator is `boot-raw-vulkan-physical-device-count-query-no-destroy-smoke`, reusing the same bundle and visible contract but stopping after the null-pointer count query and before any explicit `vkDestroyInstance` cleanup
  - reviewer read: this new no-destroy split isolates the raw count query from explicit cleanup; if it still only reaches two pulses, the active failing seam is the count query itself, and if it returns with three pulses, the cleanup path becomes the likely culprit
